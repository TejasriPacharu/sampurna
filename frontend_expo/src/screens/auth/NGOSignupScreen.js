...
// ── Light theme accent (NGO = blue) ─────────────────────────────
const ACCENT    = '#2563EB';
const ACCENT_BG = '#EFF6FF';
const ACCENT_BD = '#BFDBFE';

// ── NGO type options ─────────────────────────────────────────────
const NGO_TYPES = [
  'Food Bank', 'Orphanage', 'Old Age Home',
  'Shelter Home', 'Community Kitchen', 'School / Education', 'Other',
];

// ── Validation ───────────────────────────────────────────────────
const validate = (form, proofFile) => {
  const errs = {};
  if (!form.ngo_name.trim())       errs.ngo_name       = 'NGO name is required.';
  if (!form.volunteer_name.trim()) errs.volunteer_name = 'Volunteer name is required.';
  ...
  if (!proofFile)                  errs.proof          = 'Please upload your NGO registration certificate.';
  return errs;
};
...
export default function NGOSignupScreen({ navigation }) {
  ...
  const handleSignup = async () => {
    ...
    if (selectedType) data.append('ngo_type', selectedType);
      ...
      <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
        ...
        <View style={[styles.infoBanner, { borderColor: ACCENT_BD, backgroundColor: ACCENT_BG }]}>
          <Text style={[styles.infoBannerText, { color: ACCENT }]}>
            📋 Your account will be reviewed by admin before activation. This usually takes 24–48 hrs.
          </Text>
        </View>
        ...
        {/* Organisation Info */}
        <Text style={styles.sectionLabel}>Organisation Info</Text>
        <InputField
          label="NGO Name"
          value={form.ngo_name}
          onChangeText={(v) => update('ngo_name', v)}
          placeholder="e.g. Asha Foundation"
        />
        {fieldErrors.ngo_name && <Text style={styles.fieldErr}>{fieldErrors.ngo_name}</Text>}

        <InputField
          label="Volunteer Name"
          value={form.volunteer_name}
          onChangeText={(v) => update('volunteer_name', v)}
          placeholder="Your full name"
        />
        {fieldErrors.volunteer_name && <Text style={styles.fieldErr}>{fieldErrors.volunteer_name}</Text>}
        ...
        {/* NGO Type selector */}
        <Text style={styles.fieldLabel}>NGO Type (optional)</Text>
        <View style={styles.typeGrid}>
          {NGO_TYPES.map((t) => (
            <TouchableOpacity
              key={t}
              onPress={() => setSelectedType(t === selectedType ? '' : t)}
              style={[
                styles.typeChip,
                selectedType === t && { borderColor: ACCENT, backgroundColor: ACCENT_BG },
              ]}
            >
              <Text style={[styles.typeChipText, selectedType === t && { color: ACCENT, fontWeight: '700' }]}>
                {t}
              </Text>
            </TouchableOpacity>
          ))}
...
const styles = StyleSheet.create({
  ...
  typeChip:       { paddingHorizontal: 12, paddingVertical: 7, borderRadius: 20, borderWidth: 1, borderColor: '#E3E0D9', backgroundColor: '#FFFFFF' },
  typeChipText:   { fontSize: 12, color: '#6B6860' },
  // password
